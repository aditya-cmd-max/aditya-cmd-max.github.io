...
function getCurrentAppName() {
 ...
 // Add these methods to your existing ReverbitAuth class

async generateProfileLink() {
    if (!this.user || !this.userProfile) return null;
    
    try {
        // Ensure user has a username
        if (!this.userProfile.username) {
            // Generate a username from email or create one
            const username = this.user.email.split('@')[0] + Math.floor(Math.random() * 1000);
        ...
        // Create public profile entry if not exists
        const publicProfileRef = this.db.collection('publicProfiles').doc(this.userProfile.username);
        ...
        if (!publicProfileDoc.exists) {
            await publicProfileRef.set({
                ...
                isPublic: this.userProfile.isPublic || false,
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString()
            });
        }
        ...
        const profileUrl = `${baseUrl}/profile.html?username=${this.userProfile.username}`;
...
async toggleProfileVisibility() {
    ...
    try {
        const newVisibility = !this.userProfile.isPublic;
        ...
        this.userProfile.isPublic = newVisibility;
        
        this.showToast(`Profile is now ${newVisibility ? 'public' : 'private'}`, 'success');
        return newVisibility;
        
    } catch (error) {
        console.error('Error toggling profile visibility:', error);
        this.showToast('Error updating profile visibility', 'error');
        return null;
    }
}

// Add to your loadUserProfile method:
async loadUserProfile() {
    if (!this.user) return;
    ...
    try {
        ...
        if (userDoc.exists) {
            ...
            // Create default profile with public/private settings
            const displayName = this.user.displayName || 
                              this.user.email?.split('@')[0] || 
                              'User';
            const username = this.user.email.split('@')[0] + Math.floor(Math.random() * 1000);
            ...
            this.userProfile = {
                ...
                showApps: true, // Show apps by default
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString(),
                lastLogin: new Date().toISOString(),
                theme: 'auto',
            ...
            const publicProfileRef = this.db.collection('publicProfiles').doc(username);
            await publicProfileRef.set({
                userId: this.user.uid,
                username: username,
                displayName: displayName,
