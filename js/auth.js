...
class ReverbitAuth {
    ...
    applyTheme() {
        ...
        localStorage.setItem('reverbit_dark_mode', shouldBeDark.toString());
        ...
        console.log('Auth: Theme applied -', this.currentTheme, '(dark:', shouldBeDark, ')');
    }
    ...
    injectThemeControls() {
        // Add theme toggle to body for global access
        if (!document.getElementById('global-theme-toggle')) {
            const themeToggle = document.createElement('button');
            themeToggle.id = 'global-theme-toggle';
            themeToggle.className = 'global-theme-toggle';
            themeToggle.innerHTML = `
                <i class="fas ${this.currentTheme === 'auto' ? 'fa-adjust' : 
                               this.currentTheme === 'light' ? 'fa-sun' : 'fa-moon'}"></i>
            `;
    ...
    updateThemeToggleButton() {
        ...
        if (themeBtn) {
            themeBtn.innerHTML = `
                ...
                <span class="profile-menu-text">${textMap[this.currentTheme]}</span>
            `;
        }
        
        if (globalToggle) {
            globalToggle.innerHTML = `<i class="${iconMap[this.currentTheme].split(' ')[1]}"></i>`;
            globalToggle.title = `Current theme: ${this.currentTheme}`;
        }
    }
    ...
    showAvatarContextMenu(event) {
        ...
        const menuItems = [
            ...
            { icon: 'fa-palette', text: 'Change Theme', action: () => this.toggleTheme() },
    ...
    getPopupHTML() {
        ...
        // Theme settings
        const themeIcon = this.currentTheme === 'auto' ? 'fa-adjust' : 
